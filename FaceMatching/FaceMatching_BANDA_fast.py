...
for r in runs:
	...
	if expInfo['runMode']!="Practice":
            ...
            routineTimer.reset()
            #TEST TIME testtimeClock = core.MonotonicClock()  # to track the time since trials started
	...
	for thisTrial in trials:
		...
		if 'fixation' in top.image:
		    ...
		    trials.addData("allPressedKeysTime", str(allPressedKeysTime).replace(",",":"))
                    print "correct key: ", int(corr_ans), ",", int(corr_ans_Left)
                    print "keys pressed: ", allPressedKeys

                    #calculate stats
                    TotalCounter+=1
                    if key_resp_trial.corr == 1:
                        print "correct"
                        NumberCorrect+=1
                    elif key_resp_trial.keys != None:
                        print "incorrect"
                        NumberIncorrect+=1
                    else:
                        print "no response"
